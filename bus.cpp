...
bool Bus::send_msg(SPMsg msg, int priority = (PRIORITY_LEVEL - 1) / 2) {
	// find the socket
	...
	//just put into queue
	//may need a lock
	auto lock = std::make_unique_lock(mutex_queue);
...
void Bus::send_thread() {
	while (true) {
		...
		if (msg->data_slice_list.size() > 0) {
			auto it = data_slice_list.begin();
			for (int i = 0; i < msg->data_slice_list.size() - 1; ++i) {
				tag = ZMQ_SNDMORE;
				zmq_msg_t data_msg;
				zmq_msg_init_data(&data_msg, (*it).c_str(), (*it).size(), non_free_data, NULL);
				//string to char *
				while (true) {
			...
			tag = ZMQ_DONTWAIT;
			zmq_msg_t data_msg;
			zmq_msg_init_data(&data_msg, (*it).c_str(), (*it).size(), nonFreeData, NULL);
			while (true) {
				if (zmq_msg_send(&data_msg, socket, tag) == (*it).size()) break;
				if (errno == EINTR) continue;  // may be interupted by profiler
...
void Bus::recv_thread() {
	while(true) {
		...
		tracker->push_into_pending_msg(msg);
	}
}
